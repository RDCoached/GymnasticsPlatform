...
case "$1" in
  up)
    ...
    echo ""
    echo "📋 Next steps (run these locally for hot reload):"
    ...
    echo ""
    echo "🌐 Infrastructure Services (via Traefik):"
    ...
    echo ""
    echo "🚀 Apps (run locally, accessed at):"
    echo "   API:              http://localhost:5001"
    echo "   User Portal:      http://localhost:3001"
    echo "   Admin Portal:     http://localhost:3002"
    echo ""
    echo "💡 For full stack with Traefik URLs, use: docker compose up -d"
    ;;
